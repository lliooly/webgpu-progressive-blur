...
首版输入为图片、Canvas 或调用者提供的 GPUTexture；演示通过纹理位移模拟滚动。真实 DOM 捕获与博客集成属于后续工作，不能把纹理演示称为已完成的 DOM 背景滤镜。
...
- 使用单个 npm 包，核心入口和 `/dom` 适配入口分开，避免只使用 GPU 内核时引入 DOM 捕获依赖。
...
- 提供创建、更新参数、替换输入、调整尺寸、销毁的生命周期接口；DOM 适配额外提供 refresh 及错误状态。
